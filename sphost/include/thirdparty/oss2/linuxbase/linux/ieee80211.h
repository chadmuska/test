...
#if 1 /* moved to ieee80211_skbuff.h */
/**
...
{
	if (SKBC_PTR(skb)->len < 25)
		return false;
	return _ieee80211_is_robust_mgmt_frame((void *)((pskbc_t)skb)->data);
}
...
static inline int ieee80211_get_tdls_action(struct sk_buff *skb, u32 hdr_size)
{
	if (!skb_is_nonlinear(skb) &&
	    SKBC_PTR(skb)->len > (skb_network_offset(skb) + 2)) {
...
{
	struct ieee80211_mgmt *mgmt = (void *)SKBC_PTR(skb)->data;

	if (!ieee80211_is_action(mgmt->frame_control))
		return false;

	if (SKBC_PTR(skb)->len < IEEE80211_MIN_ACTION_SIZE +
