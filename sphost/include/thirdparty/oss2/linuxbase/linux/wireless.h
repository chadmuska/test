...
/* Andy.Li: Not copy the whole priv cmds. Only return address of the priv cmd array.
 * Usage:
	struct iwreq iwr;
	...
	iwr.u.data.pointer = (caddr_t) &priv_args;
	iwr.u.data.length = 1000; // not used.
	iwr.u.data.flags = 0;

    ioctl(SIOCGIWPRIV, &iwr);
 */
